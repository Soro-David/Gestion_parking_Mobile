...
import 'package:parking_mobile/features/agent/presentation/providers/agent_versement_provider.dart';
...
class AgentDetailVersementScreen extends StatefulWidget {
	...
	@override
	State<AgentDetailVersementScreen> createState() => _AgentDetailVersementScreenState();
}

class _AgentDetailVersementScreenState extends State<AgentDetailVersementScreen> {
	late Future<VersementDetail> _futureDetail;

	@override
	void initState() {
		super.initState();
		_futureDetail = AgentVersementProvider.repository.getVersementDetail(widget.versementId);
	}
		...
		return Scaffold(
			...
			body: FutureBuilder<VersementDetail>(
				...
				builder: (context, snapshot) {
					...
					final isValide = detail.status.toLowerCase() == 'validé' || detail.status.toLowerCase() == 'valide' || detail.status.toLowerCase() == 'paid';
					final statusColor = isValide ? Colors.greenAccent : Colors.orangeAccent;
					final statusIcon = isValide ? Icons.check_circle_rounded : Icons.pending_rounded;
													...
													child: Row(
														children: [
															Container(
																...
																child: const Icon(Icons.receipt_long_rounded, color: AppTheme.secondary, size: 18),
															),
