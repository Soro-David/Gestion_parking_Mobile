...
import 'agent_history_remote_datasource.dart';

class DioAgentHistoryRemoteDataSource implements AgentHistoryRemoteDataSource {
  DioAgentHistoryRemoteDataSource({Dio? dio})
  ...
  Future<List<ParkingEntryModel>> getEntryHistory() async {
    try {
      ...
      final response = await _api.getWithFallback(
        '/attendant/parking-sessions/history/entries',
        options: options,
      );
  ...
  Future<List<ParkingExitModel>> getExitHistory() async {
    try {
      final options = await _api.authOptions();
      final response = await _api.getWithFallback(
        '/attendant/parking-sessions/history/exits',
        options: options,
      );
